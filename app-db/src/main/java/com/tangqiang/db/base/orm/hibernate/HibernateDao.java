...
import com.tangqiang.db.base.pagination.Pagination;

/**
 * hibernate 数据库操作实现
 *
 * @author 汤强
 * @since 2013-7-30 下午7:49:24
 * @project app-db
 * @package com.tangqiang.db.base.orm.hibernate
 * @param <E> 实体类
 * @param <PK> 唯一序列号
 */
public class HibernateDao<E, PK extends Serializable> implements IHibernateDao<E, PK> {
	...
	public static IHibernateDao newInstance(SessionFactory sessionFactory, Class entityClass) throws Exception {
		if (sessionFactory == null) {
			log.error("SessionFactory 不能为空!");
			throw new Exception("SessionFactory 不能为空!");
		}
	...
	public IPagination<E> findByHql(String hql, int from, int scope, Object... params) throws Exception {
		...
		IPagination pagination = new Pagination<E>(query.list(), from, scope, this.countByHql(hql, params));
		return pagination;
	}
	...
	public IPagination<E> findBySQL(String sql, int from, int scope, Object... params) throws Exception {
		...
		IPagination pagination = new Pagination(sqlQuery.list(), from, scope, this.countByHql(sql, params));
		return pagination;
	}
	...
	public IPagination<E> findBySQL(String sql, int from, int scope, Map params) throws Exception {
		...
		IPagination pagination = new Pagination(sqlQuery.list(), from, scope, this.countBySQL(sql, params));
		return pagination;
	}
	...
	public int delete(IFilter filter) throws Exception {
		...
		if (filter != null && !filter.isFilterEmpty()) {
			StringBuffer sb = new StringBuffer(" delete from ").append(this.entityClass.getSimpleName());
			IFilterParser parser = HibernateFilterParser.newInstance();
			String condition = parser.parse(filter);
